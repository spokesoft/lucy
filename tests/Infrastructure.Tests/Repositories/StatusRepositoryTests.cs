...
public class StatusRepositoryTests : RepositoryTestBase
{
    ...
    public async Task GetByProjectIdAsync_WithSorting_ShouldReturnSortedStatuses(bool useWriteRepo)
    {
        ...
        {
            var repository = new StatusRepository(writeContext);
            statuses = await repository.GetByProjectIdAsync(project.Id, StatusField.Order, Application.Common.Queries.SortDirection.Descending);
        }
        else
        {
            await using var readContext = new LucyReadContext(_readDbContextOptions);
            var readOnlyRepo = new StatusReadOnlyRepository(readContext);
            statuses = await readOnlyRepo.GetByProjectIdAsync(project.Id, StatusField.Order, Application.Common.Queries.SortDirection.Descending);
        }
